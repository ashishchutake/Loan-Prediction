...
df = pd.read_csv(r"C:\Users\UIX\Desktop\Ashish\projects\LoanPrediction\Data\train.csv")
...
####################################################3
df_train_X = df_train.iloc[:,0:10].values   

df_train_Y = df_train.iloc[:,11].values   
...
train_X,test_X,train_Y,test_Y = train_test_split(df_train_X,df_train_Y,test_size=0.3,shuffle=True)

predictedRF = RFmodel.fit(train_X,train_Y)

result = predictedRF.predict(test_X)
result

RFmodel.score(train_Y,test_X)


# Train and Test Accuracy
print("Train Accuracy :: ", accuracy_score(train_Y, predictedRF.predict(train_X)))
